// Platform test and session labels.
...
export const TAG_LABELS: Record<PlatformTag, { label: string; hint: string }> = {
	basic: { label: 'Basic', hint: 'Commonly shared media platforms.' },
	nsfw: { label: 'Adult', hint: 'Platforms with adult content.' },
	news: { label: 'News', hint: "News outlets and broadcasters' news programmes." },
	social: { label: 'Social', hint: 'Social networks and forums.' },
...
export const SESSION_LABELS: Record<SessionSupport, { label: string; hint: string }> = {
	...
	optional: {
		...
		hint: 'Login may be needed for private, age-restricted or higher-quality media.'
	},
	required: { label: 'Account needed', hint: 'Login required to download media.' }
};
...
/** Summarize the latest platform test run. */
...
export function sessionSummary(platform: PlatformCoverage): {
	...
	if (check?.state === 'unsupported') return { label: 'Session checks unavailable', tone: 'neutral' };
	if (platform.cookies === 0) {
		return platform.session === 'required'
			? { label: 'Import cookies to use this platform', tone: 'warn' }
